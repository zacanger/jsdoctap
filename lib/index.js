...
const toJsdocRegister = (comment) => {
  ...
  const compiled = _.map(comment.examples, (example) => {
    ...
    const fn = 'function() {' + example.testCase + '}'
...
const run = (filename) => {
  ...
  const ts = getTests(content)

  content += _.map(ts, toJsdocRegister).join('')
...
// Making someone do `tap --node-arg=-r jsdoctap src/**/*.js` is just stupid.
// We should just act as the test runner and use tap under the hood.
try {
  if (path.basename(process.argv[1]) === '_tap') {
    let tap = require('./tap')
    if (module.parent.exports === tap) {
      delete require.cache[path.join(__dirname, 'tap.js')]
      tap = require('./tap')
    }
